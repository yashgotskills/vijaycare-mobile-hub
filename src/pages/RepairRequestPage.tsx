...
const RepairRequestPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.rpc("create_repair_request" as any, {
        _user_phone: userPhone,
        _customer_name: formData.name,
        _device_type: formData.deviceType,
        _brand: formData.brand,
        _model: formData.model || null,
        _repair_type: selectedRepair?.name || formData.repairType,
        _issue_description: formData.issueDescription || null,
        _address: formData.address,
        _preferred_date: formData.preferredDate,
        _preferred_time: formData.preferredTime,
      });
